...
|-------|---------|-------------|
| 2026-05-21 | **3.0.3** | **UserProfilePopover — slot-machine animation + continuous dice spin**: Efecto slot-machine en la grilla de héroes al ejecutar random: el foco (highlight ámbar con glow) salta de héroe en héroe con deceleración exponencial (30ms→180ms/paso) durante 1300ms, se detiene en el ganador 200ms y finaliza actualizando avatar + bounce pop. Dado gira 4 rotaciones completas (1440°) en 1500ms con `ease-out` (empieza rápido, desacelera al final). `HeroGrid` acepta `scanningId` prop: héroe activo recibe `ring-2 ring-amber-400 + glow shadow`, resto se atenúa al 40%. Avatar grande dispara `animate-avatar-pop` (bounce 0.5s) al aterrizar el ganador. `handleRandomize` movido antes del early return (fix regla de hooks). `randomizeHero` del store reemplazado por lógica local con `updateAvatar`. |
| 2026-05-21 | **3.0.2** | **UserProfilePopover — botón Random inline + animaciones**: Botón dado (Dices) movido como icono compacto **a la derecha del lápiz** de edición dentro del row de nombre (eliminado el botón full-width separado). Al ejecutar random: dado gira 360° con efecto bounce (`@keyframes dice-roll`) y el avatar grande hace pop de escala+rotación (`@keyframes avatar-pop`), ambas animaciones de 600-650ms en `globals.css`. `Button` shadcn reemplazado por `<button>` nativo en el save del modo edición (eliminado import). `useCallback` consolidado en el import de React. SVGs: confirmado que todos los avatares hero son SVGs inline — no se requieren archivos de imagen externos. |
| 2026-05-17 | **3.0.1** | **BPMN Studio — Token Simulation + YAML enriquecido + Editor YAML inline**: Integración de `bpmn-js-token-simulation` v0.39 como módulo adicional del editor BPMN. Botón **▶ Simular** (violeta) / **■ Detener** (ámbar) en `StudioToolbar` que activa/desactiva el modo simulación de tokens. Banner "Modo Simulación activo" en el canvas al activarse. `toggleSimulation` / `isSimulating` expuestos en `BpmnModelerHandle`. Declaración de tipos `types/bpmn-js-token-simulation.d.ts`. **Auto-sync configurable**: checkbox `Auto` en header de `YamlPreview` (off por defecto); el polling 800ms solo corre cuando está activo, eliminando carga continua de CPU. **YAML enriquecido**: `bpmn-to-yaml.ts` ahora emite siempre todos los campos del esquema con defaults visibles: `description` en tareas y fases, `type` explícito (standard/check/multicheck/export-excel/…), `evidence.description`, `completionAlert` con scaffold completo (severity/title/description/confirmLabel/cancelLabel), `variables: []` a nivel proceso. **Editor YAML inline**: `YamlPreview` modo lectura/edición — botón ✏️ `Editar` carga el YAML en `<textarea>` editable (Tab = 2 espacios, validación en tiempo real); botón `✅ Aplicar` guarda cambios al estado, `❌ Cancelar` descarta. `onYamlChange` prop en `YamlPreview` notifica cambios manuales al padre; Auto-sync y Sync se ocultan durante edición. |
| 2026-05-16 | **3.0.0** | **BPMN Studio Editor**: Nueva ruta `/studio` con editor visual BPMN bidireccional. Botón "Studio" en header del home. `BpmnModeler` (bpmn-js Modeler, SSR-off) con canvas editable, keyboard shortcuts, zoom in/out/fit. `YamlPreview` con sintaxis highlight, validación en tiempo real y botones copiar/descargar. `StudioToolbar` con acciones: Nuevo proceso, Importar BPMN XML, Cargar del catálogo, Exportar XML, Exportar YAML. `ImportXmlModal` con tabs paste/file (.bpmn/.xml). `CatalogModal` para cargar cualquier proceso del catálogo → convierte YAML→BPMN y lo abre en el editor. `lib/bpmn-to-yaml.ts`: transformador inverso BPMN XML → YAML (parseBpmnXml, bpmnToYaml, validateYamlString). Panel split redimensionable (react-resizable-panels). 27 nuevos tests unitarios en `bpmn-to-yaml.test.ts`. |
...
| 2026-04-23 | **2.1.3** | **Detail Table + Cell Export + Sheets Architecture + Output Variables**: Nuevo tipo de tarea `detail-table` para capturar datos estructurados por item en formato tabla. Tipos de columna: `boolean`, `date`, `list`, `text`, `computed-text`. Componente `DetailTableInput` con tabla responsive, validación de campos requeridos. Exportación Excel `kind: detail-table` con mapeo `columns` + `startRow`. Labels con sintaxis `#CELL#` resueltos desde template Excel. **Nuevo `kind: cell`**: exporta cualquier campo de cualquier tarea a una celda Excel específica usando dot-notation path. **Refactorización mayor: arquitectura `sheets[]`** en `ProcessExportMappings`. Eliminados campos planos y `export.sheet`. Todo se agrupa en `sheets: ExportSheetSection[]` con `sources: ExportSource[]`. Source kinds: `variables`, `static`, `time`, `process`, `comments`, `range`, `list`, `detail`, `form`, `checklist`, `detail-table`, `cell`. **Variables de salida de tareas (`outputVars`)**: cualquier tarea puede declarar `outputVars` que se escriben en `capturedVariables` al completarse. Tipos: `text`, `list`, `object`. Source: dot-notation path sobre TaskState (ej: `listData`, `evidence.text`). `mapTo` para extraer propiedad de cada item en listas. **`CapturedVariables` ahora soporta `string[]`** (Opción B). **`optionsFrom` en variables de proceso**: variable tipo `select` puede obtener opciones dinámicamente desde una `capturedVariable` que contenga una lista. **`sourceVar` en `detailTableConfig`**: alternativa a `sourceTaskId`, lee items desde una variable de proceso. **`kind: range`**: lee rango de celdas del template Excel y almacena en `capturedVariables`. Helper `colLetterToNumber`. 68 tests unitarios (23 output-vars + 45 previos).n `timestampColumn`/`nameColumn` (antes `dateColumn`/`activityColumn`). `mergeMappings` fusiona por nombre de hoja. `executeExportPlan` itera sheets[]. `resolveExportPlan` sin `sheet`. `task-card.tsx` y `word-generator.ts` leen sheet de `sheets[0].sheet`. 45 tests unitarios pasando. **Confirmación de cierre + consolidación documental**. *Feature:* bloque opcional `completionAlert` en cualquier tarea (`severity: info|warning|critical`, `title`, `description`, `confirmLabel`, `cancelLabel`), renderizado mediante `CompletionAlertDialog` con estilos derivados de `lib/alert-feedback.ts` (animaciones `pulse-once`/`pulse-strong`, respeto de `prefers-reduced-motion`, i18n `common.confirm` / `alert.completion.defaultTitle`). Integrado en `TaskCard` como gate previo al cierre; cancelar preserva el estado anterior. *Bugfixes:* estilos de botones de alerta — confirmar usa `bg-green-500 text-white` (uniforme con "Terminar tarea"), cancelar usa `bg-gray-*` según severidad; removido `buttonVariants()` de `AlertDialogAction/Cancel` para permitir estilos custom; fondo `bg-background` removido de `AlertDialogContent` para permitir `containerClass` por severidad. *Bugfix:* doble alerta al completar desde modal de evidencia — flag `completionAlertAlreadyConfirmed` propagado vía `onViewEvidence(taskId, alertAlreadyConfirmed)` desde `TaskCard` → `ActivityCard` → `page.tsx` → `EvidenceModal`; usa `useRef` (no `useState`) para acceso síncrono en callback. *Bugfix:* hydration mismatch en `page.tsx` — estado `mounted` + `suppressHydrationWarning`. *Bugfix:* favicon 404 — `icons: { icon: '/icon.svg' }` en metadata. *Bugfix:* timer auto-resume después de pausa manual — `wasManuallyPaused` ref resetea `hasAutoStarted` permitiendo re-auto-start en siguiente interacción. *Schema:* `schemas/process.schema.json` bump a **v1.1.0** (`$id` versionado, `$comment` con fecha de sync, patrón `Duration` alineado con `parseTimeString` — añade decimales y `s`, elimina `d` no soportado, patrón `Process.version` con SemVer completo incluyendo pre-release/build). *Datos:* armonización de IDs entre `data/processes/index.json` y YAMLs internos (sufijo `-2026`), propagación de `estimatedTime` y `hasVariables` a todas las entradas del catálogo, interfaces `ProcessTemplate` extendidas en `app/page.tsx` y `app/api/processes/route.ts`. *Docs:* reescritura integral de `README.md`, `README.dockerhub.md`, `README.process.md` y `README.bpmn.md`; nuevo diagrama **C4** (`docs/diagrams/c4-model.md`) con tres niveles (Context / Container / Component); reescritura de `arquitectura-sistema.md`, `flujo-proceso.md` y `flujo-datos.md`. *Testing:* 580+ tests unitarios pasando en 27 archivos (nuevos suites `completion-alert-dialog`, `alert-feedback` ampliado con `confirmButtonClass`/`cancelButtonClass`/`containerClass`, `task-card` doble alerta + `markInteractionStarted`, `process-timer-auto-start` auto-resume post-pause); `npm run validate:processes` valida los 10 YAMLs (6 productivos + 4 fixtures). |
...
### Features (feat)
- Tipos de tareas: dynamic-list, detail-list, detail-table, form
...
### Cobertura de Tests
- **Tests Unitarios**: 545+ en 28 archivos (Vitest 4)
...
- Unreleased: completionAlert + schema v1.2.0 + outputVars + optionsFrom + sourceVar + kind:range + docs overhaul + C4
